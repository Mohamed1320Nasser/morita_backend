  ...
  try {
    ...
    const agilityService = await prisma.service.upsert({
      ...
      create: {
        ...
        slug: 'agility',
        emoji: '🏃',
        description: 'Professional Agility training with optimal routes',
        active: true,
        displayOrder: 1,
      },
    });
    ...
    for (const method of agilityPricingMethods) {
      ...
      console.log(`    ✓ ${method.name}: ${method.startLevel}-${method.endLevel} = $${method.basePrice}/XP`);
    }
    console.log('');
    ...
    const fireCapeService = await prisma.service.upsert({
      ...
      create: {
        ...
        slug: 'fire-cape',
        emoji: '🔥',
        description: 'Professional Fire Cape service with various account types',
        active: true,
        displayOrder: 1,
      },
    });
    ...
    const upchargeModifiers = [
      {
        ...
        value: 20,
        description: 'Additional charge if Rigour prayer is not unlocked',
      },
      {
    ...
    console.log(`  curl http://localhost:3000/api/public/services/${agilityService.id}/with-pricing`);
    console.log(`  curl http://localhost:3000/api/public/services/${fireCapeService.id}/with-pricing\n`);
