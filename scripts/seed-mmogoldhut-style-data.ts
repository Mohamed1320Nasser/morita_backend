...
import { PrismaClient, PricingUnit, ModifierDisplayType } from '@prisma/client';
...
async function seedSkillsCategory(categoryId: string) {
  ...
  const agility = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'agility',
      emoji: '🏃',
      description: 'Professional Agility training using optimal rooftop courses. Fast leveling with marks of grace collection included.',
      active: true,
      displayOrder: 1,
    },
  });
  ...
  const agilityMethods = [
    { name: 'Agility', startLevel: 1, endLevel: 40, basePrice: 0.00007, order: 1 },
    { name: 'Canifis Rooftop', startLevel: 40, endLevel: 50, basePrice: 0.000054, order: 2 },
    { name: 'Falador Rooftop', startLevel: 50, endLevel: 60, basePrice: 0.000045, order: 3 },
    { name: 'Seers Rooftop With Diary', startLevel: 60, endLevel: 90, basePrice: 0.00002, order: 4 },
    { name: 'Seers Rooftop Without Diary', startLevel: 60, endLevel: 90, basePrice: 0.000023, order: 5 },
    { name: 'Rellekka Rooftop', startLevel: 80, endLevel: 99, basePrice: 0.000024, order: 6 },
    { name: 'Ardougne Rooftop', startLevel: 90, endLevel: 99, basePrice: 0.00002, order: 7 },
    { name: 'Wilderness', startLevel: 52, endLevel: 60, basePrice: 0.00002, order: 8 },
  ];
  ...
  for (const method of agilityMethods) {
    ...
    agilityMethodIds.push(created.id);
  }
  ...
  console.log(`    ✓ Created ${agilityMethods.length} rooftop course pricing methods with Marks of Grace note`);
  ...
  const runecrafting = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'runecrafting',
      emoji: '🔮',
      description: 'Runecrafting training through various methods including ZMI, Lavas, and Bloods.',
      active: true,
      displayOrder: 2,
    },
  });
  ...
  const rcMethods = [
    { name: 'ZMI Altar (No requirements)', startLevel: 1, endLevel: 77, basePrice: 0.00013, order: 1 },
    { name: 'Lava Runes (Fast XP)', startLevel: 23, endLevel: 99, basePrice: 0.00018, order: 2 },
    { name: 'Blood Runes (77-99 AFK)', startLevel: 77, endLevel: 99, basePrice: 0.00008, order: 3 },
  ];
  ...
  console.log(`    ✓ Created ${rcMethods.length} methods with upcharges and notes`);
  ...
  const smithing = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'smithing',
      emoji: '🔨',
      description: 'Smithing training through gold bar smelting. Fast and AFK-friendly method.',
      active: true,
      displayOrder: 3,
    },
  });
...
async function seedMinigamesCategory(categoryId: string) {
  ...
  const fireCape = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'fire-cape',
      emoji: '🔥',
      description: 'Professional Fire Cape service with Parsec/VPN options. Guaranteed completion or money back.',
      active: true,
      displayOrder: 1,
    },
  });
  ...
  console.log(`    ✓ Created ${fireCapeTiers.length} tiers with ${upcharges.length} upcharges and ${notes.length} notes`);
  ...
  const infernalCape = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'infernal-cape',
      emoji: '🌋',
      description: 'Expert Infernal Cape service. Our specialists have 100+ completions. Parsec only for security.',
      active: true,
      displayOrder: 2,
    },
  });
...
async function seedQuestsCategory(categoryId: string) {
  ...
  const rfd = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'recipe-for-disaster',
      emoji: '🍲',
      description: 'Complete all RFD subquests and unlock Barrows gloves. Prerequisites included.',
      active: true,
      displayOrder: 1,
    },
  });
...
async function seedBossingCategory(categoryId: string) {
  ...
  const zulrah = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'zulrah',
      emoji: '🐍',
      description: 'Zulrah kill service. Perfect for learning or grinding pet/uniques.',
      active: true,
      displayOrder: 1,
    },
  });
  ...
  const cox = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'chambers-of-xeric',
      emoji: '⚔️',
      description: 'CoX raid boosting service. Experienced raiders with fast completion times.',
      active: true,
      displayOrder: 2,
    },
  });
...
async function seedGoldCategory(categoryId: string) {
  ...
  const gold = await prisma.service.upsert({
    ...
    create: {
      ...
      slug: 'osrs-gold',
      emoji: '💵',
      description: 'Buy OSRS gold at competitive rates. Safe and fast delivery.',
      active: true,
      displayOrder: 1,
    },
  });
...
async function main() {
  ...
  try {
    ...
    console.log('  ✓ All pricing units (FIXED, PER_LEVEL, PER_HOUR, PER_KILL, PER_ITEM)');
