...
export async function handlePaymentAll(interaction: ButtonInteraction): Promise<void> {
    try {
        ...
        logger.info(`[PaymentAll] Payment methods shown to ${interaction.user.tag}`);
    } catch (error) {
        logger.error("[PaymentAll] Error:", error);
        await interaction.reply({
