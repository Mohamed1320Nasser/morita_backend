...
export async function handleResetCalculator(
    ...
    try {
        ...
        logger.info(`Calculator reset by ${interaction.user.tag}`);
    } catch (error) {
        logger.error("Error handling reset calculator button:", error);
        await interaction.editReply({
