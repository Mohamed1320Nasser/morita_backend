...
export async function handleSubmitAccountJagexLauncher(interaction: ButtonInteraction): Promise<void> {
    try {
        const orderId = interaction.customId.replace("submit_account_jagex_launcher_", "");
        ...
        // Show modal for Jagex Launcher account type
        const modal = new ModalBuilder()
            .setCustomId(`account_data_modal_jagex_launcher_${orderId}`)
            .setTitle("🚀 Jagex Launcher Account");
        ...
        const backupCodesInput = new TextInputBuilder()
            ...
            .setLabel("Backup Codes")
            .setStyle(TextInputStyle.Paragraph)
            .setRequired(false)
            .setMaxLength(500)
            .setPlaceholder("Enter backup codes if available");
        ...
        logger.info(`[SubmitAccountJagexLauncher] Showing modal for order ${orderId} to ${interaction.user.id}`);
    } catch (error: any) {
        logger.error("[SubmitAccountJagexLauncher] Error:", error);
