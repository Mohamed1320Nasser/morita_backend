...
export async function handleAccountDataJagexLauncherModal(interaction: ModalSubmitInteraction): Promise<void> {
    try {
        await interaction.deferReply({ ephemeral: true });

        // customId format: account_data_modal_jagex_launcher_{orderId}
        const orderId = interaction.customId.replace("account_data_modal_jagex_launcher_", "").substring(0, 36);

        logger.info(`[AccountDataJagexLauncherModal] Parsed - orderId: ${orderId}`);

        // Collect all field values from the Jagex Launcher form
        const accountData: Record<string, string> = {};
        const fieldNames = ["username", "password", "bank_pin", "bank_value", "backup_codes"];
        ...
        const embed = new EmbedBuilder()
            ...
            .setDescription(
                `Your **Jagex Launcher** account data has been securely submitted for **Order #${order.orderNumber}**.\n\n` +
                `The worker or support will be able to view this data **one time only**.`
            )
            .addFields([
                { name: "Account Type", value: "🚀 Jagex Launcher", inline: true },
        ...
        try {
            if (interaction.channel) {
                    ...
                    if (msg.author.id === interaction.client.user?.id) {
                        ...
                        if (hasAccountTypeButtons) {
                            const disabledEmbed = new EmbedBuilder()
                                .setTitle("✅ Account Data Submitted")
                                .setDescription(`**Jagex Launcher** account data has been submitted for **Order #${order.orderNumber}**.`)
                                .setColor(0x57f287)
                                .setTimestamp();
                ...
                const notifyEmbed = new EmbedBuilder()
                    .setTitle("🔐 Account Data Submitted")
                    .setDescription(`<@${interaction.user.id}> has submitted their **Jagex Launcher** account data for **Order #${order.orderNumber}**.`)
                    .addFields([
            ...
            logger.error("[AccountDataJagexLauncherModal] Failed to send notification:", err);
        }
    } catch (error: any) {
        logger.error("[AccountDataJagexLauncherModal] Error:", error);
